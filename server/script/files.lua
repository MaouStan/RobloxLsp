...
function m.getRequires(uri)
    if m.requireMap[uri] then
        return m.requireMap[uri]
    end
    local ast = m.getAst(uri)
    if ast and ast.ast and ast.ast.requires then
        local map = {}
        local vm = require("vm")
...
function m.getRequiring(reqUri, recursive, list, mark)
    ...
    for _, uri in ipairs(m.getAllUris()) do
        if not mark[uri] and uri ~= reqUri then
            local requires = m.getRequires(uri)
...
function m.compileAst(uri, text)
    ...
    local state, err = parser:compile(text, 'lua', uri)
