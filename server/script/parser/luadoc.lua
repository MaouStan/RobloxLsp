...
local function parseTypecheck()
    ...
    result.finish = getFinish()
    if  mode ~= 'mode' then
    -- and mode ~= 'mode-next-line' then
        pushError {
            type   = 'LUADOC_ERROR_TC_MODE',
            start  = result.start,
            finish = result.finish,
        }
    end
    ...
    if checkToken('symbol', ':', 1) then
        ...
        result.names = {}
        while true do
            local name = parseName('doc.typecheck.name', result)
            if not name then
                pushError {
                    type   = 'LUADOC_MISS_TC_NAME',
                    start  = getFinish(),
                    finish = getFinish(),
                }
                return nil
